...
all shells are listed in the ```run_uda.sh```.
...
Change related path in ```config/demo_uda.json``` as follows:
```
"sup_data_dir": "demo/imdb_sup_train.txt",
"unsup_data_dir": "demo/imdb_unsup_train.txt",
"eval_data_dir": "demo/imdb_sup_test.txt",
``` 
