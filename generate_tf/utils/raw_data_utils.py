...
class IMDbProcessor(DataProcessor):
  ...
  def get_train_examples(self, raw_data_dir):
    ...
    return self._create_examples(
        self._read_tsv(raw_data_dir, # os.path.join(raw_data_dir, "train.csv"), #TODO change
                       quotechar='"'), "train")

  def get_dev_examples(self, raw_data_dir):
    """See base class."""
    return self._create_examples(
        self._read_tsv(raw_data_dir, #os.path.join(raw_data_dir, "test.csv"), # TODO CHANGE
                       quotechar='"'), "test")
  ...
  def get_unsup_examples(self, raw_data_dir, unsup_set):
    ...
    if unsup_set == "unsup_ext":
      return self._create_examples(
          self._read_tsv(raw_data_dir, # os.path.join(raw_data_dir, "unsup_ext.csv"), # TODO change
  ...
  def _create_examples(self, lines, set_type, skip_unsup=True):
    ...
    for (i, line) in enumerate(lines):
      ...
      #   continue
      # TODO CHANGE
      if "unsup" in set_type:
        guid = "%s" % set_type
        text_a = line[0]
        label = "unsup"
      else:
        guid = "%s" % set_type # "%s-%s" % (set_type, line[2]) TODO CHANGE
        text_a = line[1]  # TODO change
        label = line[0]
